...
bool ADXL345_WE::init()
{
    ...
    xTaskCreatePinnedToCore(ADXL345_IntHandle_Task, "ADXL345_IntHandle_Task", 4096, this, 20, &Test_taskHandle,
                            APP_CPU_NUM);
...
void ADXL345_WE::setInactivityParameters(adxl345_dcAcMode mode, adxl345_actTapSet axes, float threshold,
                                         uint8_t inactTime)
{
    regVal = static_cast<uint8_t>(round(threshold / 0.0625));
    if (regVal < 1)
...
String ADXL345_WE::getActTapStatusAsString()
{
    ...
    if (regVal & (1 << ADXL345_TAP_Z))
    {
        returnStr += "TAP-Z ";
    }
    if (regVal & (1 << ADXL345_TAP_Y))
    {
        returnStr += "TAP-Y ";
    }
    if (regVal & (1 << ADXL345_TAP_X))
    {
        returnStr += "TAP-X ";
    }
    if (regVal & (1 << ADXL345_ACT_Z))
    {
        returnStr += "ACT-Z ";
    }
    if (regVal & (1 << ADXL345_ACT_Y))
    {
        returnStr += "ACT-Y ";
    }
    if (regVal & (1 << ADXL345_ACT_X))
    {
        returnStr += "ACT-X ";
    }
...
{
    esp_err_t err = i2c_bus_write_bytes(i2c_bus.device, reg, 1, (unsigned char *)&val);
    return err;
}
